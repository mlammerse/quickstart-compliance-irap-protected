...
try:
    ...
    logger.info("failed to connect to S3")
    sys.exit(1)
...
def download_ip_list(s3_bucket, object_key):
    ...
    try:
        ...
        logger.info("Failed to download IP list from S3. "
...
def describe_target_health(tg_arn):
    ...
    try:
        ...
        logger.info("Number of currently registered IP: ", registered_ip_count)
...
def lambda_handler(event, context):
    ...
    if MAX_LOOKUP_PER_INVOCATION <= 0:
        logger.error("MAX_LOOKUP_PER_INVOCATION is negative or zero, try again")
        sys.exit(1)
    if INVOCATIONS_BEFORE_DEREGISTRATION  <= 0:
        logger.error("INVOCATIONS_BEFORE_DEREGISTRATION  is negative or zero, try again")
        sys.exit(1)
    ...
    logger.info("Authoritative name server: {}".format(authoritative_server_ip_list))
    ...
    while i <= MAX_LOOKUP_PER_INVOCATION:
        ...
        if len(dns_lookup_result) < 8:
            break
        i+=1
    logger.info("IPs detected by DNS lookup:", regular_record_set)
    logger.info("Number of IPs detected by DNS lookup: ", len(regular_record_set))
    ...
    if not regular_record_set:
        logger.error("The number of IPs in DNS for the ALB is"
              " showing up as zero. This cannot be correct.")
        logger.error("Script will not proceed with "
    ...
    logger.info("Active IPs from last invocation: {}".format(old_active_ip_dict))
    logger.info("Pending deregistration IP from last invocation: {}".format(old_pending_ip_dict))
    logger.info("Active IPs from the current invocation {}".format(new_active_ip_dict))
    ...
    if old_active_ip_dict:
        ...
        logger.info("Pending deregistration IPs from current invocation - {}".\
            format(deregiter_ip_diff_set))
        if old_pending_ip_dict:
            old_pending_ip_set = set(old_pending_ip_dict.keys())
            logger.info("Pending deregistration IPs from last invocation - {}" \
            ...
            logger.info("Additional pending IPs " \
            ...
            logger.info("Existing pending IPs (pending " \
            ...
            logger.info("Missing pending IPs (pending " \
        ...
        logger.info("New pending deregisration IP- {}" .format(new_pending_ip_dict))
    else:
        logger.info("No active IP List from last invocation")
    ...
    if registration_ip_list:
        ...
        logger.info("Registering {}".format(registration_ip_list))

    else:
        logger.info("No new target registered")
